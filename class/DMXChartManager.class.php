...
class DMXChartManager
{
    ...
    public function GetFixtureDates(DMXChart $DMXChart)
    {
        ...
        $prep->closeCursor();
        $prep = null;
        return (is_array($data)) ? $this->ConvertDate((!empty($data[1]['lastdate'])) ? $data[1]['lastdate'] : $data[0]['lastdate']) : false;
    }
        ...
        if ($type == 'table') {
            ...
            for ($i = 1; $i <= 512; $i++) {
                ...
                if ($dmx != null) {
                    $content .= "\t\t" . '<tr>' . "\n";
                    $content .= "\t\t\t" . '<td>' . $i . '</td>' . "\n";
                    $content .= "\t\t\t" . '<td data-option="' . strtolower($dmx) . '"><span>' . $dmx . '</span></td>' . "\n";
    ...
    {
        $query = 'SELECT COUNT(DISTINCT(`Manufacturer`)) AS Manufacturer, COUNT(`id`) AS Mode FROM `Fixtures` WHERE `lastdate` = :lastdate;';
        $prep = $this->_db->prepare($query);
        $prep->bindValue(':lastdate', _DateLibrary_, PDO::PARAM_STR);
        $prep->execute();
        $data = $prep->fetch(PDO::FETCH_ASSOC);
        ...
        $NbFixtures = $this->CountFixtureDate(_DateLibrary_);
        ...
        $query = 'SELECT `Manufacturer`, `Fixture`, COUNT(`mode`) AS `NbMaxMode` FROM `Fixtures` WHERE `lastdate` = :lastdate GROUP BY `Manufacturer`, `Fixture` ORDER BY `NbMaxMode` DESC LIMIT 1 ;';
        $prep = $this->_db->prepare($query);
        $prep->bindValue(':lastdate', _DateLibrary_, PDO::PARAM_STR);
        $prep->execute();
        $data = $prep->fetch(PDO::FETCH_ASSOC);
