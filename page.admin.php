	...
	$DebugFixture = 'ADA 75s';
		...
		function fileExists($fileName, $caseSensitive=true) {
			...
			if(file_exists($fileName)) {
				//File Found
				if(!sPreset) {
					//$DMXChartManager->QualityScan('OK', strtoupper(substr(strrchr($fileName, '.'), 1)), '', '', '', $fileName);
				}
				return $fileName;
			}
			if($caseSensitive) {
		...
		function SaveDMXChart($infos, $xml) {
			global $DMXChartManager, $BeforeBreak, $FolderLib;
			$TotalChannel = 0;
			$TotalWheel = 0;
			$PrevWheelName = '';
			$iColor = 0;
			...
			//$BeforeBreak++; if($BeforeBreak == 300) { die(); }
